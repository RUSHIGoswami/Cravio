...
- **api** — only runs when `api/**` changed: `ruff check` + `pytest` on Python 3.12, deps installed via `uv sync --group dev` against the committed `api/uv.lock` (`api/pyproject.toml` — single source of truth, no separate requirements file).
